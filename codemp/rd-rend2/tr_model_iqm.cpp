...
qboolean R_LoadIQM( model_t *mod, void *buffer, int filesize, const char *mod_name ) {
	...
	if( header->version != IQM_VERSION ) {
		R_Printf(PRINT_WARNING, "R_LoadIQM: %s is a unsupported IQM version (%d), only version %d is supported.\n",
	...
	if ( header->num_joints > IQM_MAX_JOINTS ) {
		R_Printf(PRINT_WARNING, "R_LoadIQM: %s has more than %d joints (%d).\n",
	...
	for( i = 0; i < header->num_meshes; i++, mesh++) {
		...
		{
			R_Printf(PRINT_WARNING, "R_LoadIQM: %s has more than %i verts on %s (%i).\n",
		...
		{
			R_Printf(PRINT_WARNING, "R_LoadIQM: %s has more than %i triangles on %s (%i).\n",
	...
	if( header->num_poses != header->num_joints && header->num_poses != 0 ) {
		R_Printf(PRINT_WARNING, "R_LoadIQM: %s has %d poses and %d joints, must have the same number or 0 poses\n",
...
void R_AddIQMSurfaces( trRefEntity_t *ent, int entityNum ) {
	...
	if ( (ent->e.frame >= data->num_frames) 
	     ...
	     || (ent->e.oldframe >= data->num_frames)
	     || (ent->e.oldframe < 0) ) {
		R_Printf( PRINT_DEVELOPER, "R_AddIQMSurfaces: no such frame %d to %d for '%s'\n",
