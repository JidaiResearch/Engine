...
void R_RemapShader(const char *shaderName, const char *newShaderName, const char *timeOffset) {
	...
	if (sh == NULL || sh == tr.defaultShader) {
		R_Printf( PRINT_WARNING, "WARNING: R_RemapShader: shader %s not found\n", shaderName );
		return;
	}
	...
	if (sh2 == NULL || sh2 == tr.defaultShader) {
		R_Printf( PRINT_WARNING, "WARNING: R_RemapShader: new shader %s not found\n", newShaderName );
		return;
	}
...
static qboolean ParseVector( const char **text, int count, float *v ) {
	...
	if ( strcmp( token, "(" ) ) {
		R_Printf( PRINT_WARNING, "WARNING: missing parenthesis in shader '%s'\n", shader.name );
		return qfalse;
	}
	...
	for ( i = 0 ; i < count ; i++ ) {
		token = COM_ParseExt( text, qfalse );
		if ( !token[0] ) {
			R_Printf( PRINT_WARNING, "WARNING: missing vector element in shader '%s'\n", shader.name );
			return qfalse;
		}
		v[i] = atof( token );
	}
...
static int NameToSrcBlendMode( const char *name )
{
	...
	R_Printf( PRINT_WARNING, "WARNING: unknown blend mode '%s' in shader '%s', substituting GL_ONE\n", name, shader.name );
	return GLS_SRCBLEND_ONE;
}
...
static genFunc_t NameToGenFunc( const char *funcname )
{
	...
	R_Printf( PRINT_WARNING, "WARNING: invalid genfunc name '%s' in shader '%s'\n", funcname, shader.name );
	return GF_SIN;
}
...
static void ParseWaveForm( const char **text, waveForm_t *wave )
{
	...
	if ( token[0] == 0 )
	{
		R_Printf( PRINT_WARNING, "WARNING: missing waveform parm in shader '%s'\n", shader.name );
		return;
	}
...
static void ParseTexMod( const char *_text, shaderStage_t *stage )
{
	...
	if ( stage->bundle[0].numTexMods == TR_MAX_TEXMODS ) {
		R_Error( ERR_DROP, "ERROR: too many tcMod stages in shader '%s'", shader.name );
		return;
	}
	...
	if ( !Q_stricmp( token, "turb" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing tcMod turb parms in shader '%s'\n", shader.name );
			return;
		}
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing tcMod turb in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "scale" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing scale parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "scroll" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing scale scroll parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "stretch" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing stretch parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "transform" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing transform parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "rotate" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing tcMod rotate parms in shader '%s'\n", shader.name );
			return;
		}
	...
	{
		R_Printf( PRINT_WARNING, "WARNING: unknown tcMod '%s' in shader '%s'\n", token, shader.name );
	}
}
...
)
{
	...
	if (!Q_stricmp(param, "ssFademax"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite fademax in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value <= stage->ss->fadeDist)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite fademax (%.2f <= fadeDist(%.2f)) in shader '%s'\n", value, stage->ss->fadeDist, shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssFadescale"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite fadescale in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
	...
	if (!Q_stricmp(param, "ssVariance"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite variance width in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite variance width in shader '%s'\n", shader.name );
			return false;
		}
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite variance height in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite variance height in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssHangdown"))
	{
		...
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: Hangdown facing overrides previous facing in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssAnyangle"))
	{
		if (stage->ss->facing != SURFSPRITE_FACING_NORMAL)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: Anyangle facing overrides previous facing in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssFaceup"))
	{
		if (stage->ss->facing != SURFSPRITE_FACING_NORMAL)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: Faceup facing overrides previous facing in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssWind"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite wind in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0.0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite wind in shader '%s'\n", shader.name );
			return false;
		}
		stage->ss->wind=value;
	...
	if (!Q_stricmp(param, "ssWindidle"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite windidle in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0.0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite windidle in shader '%s'\n", shader.name );
			return false;
		}
		stage->ss->windIdle=value;
		return true;
	...
	if (!Q_stricmp(param, "ssVertskew"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite vertskew in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0.0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite vertskew in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssFXDuration"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite duration in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value <= 0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite duration in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssFXGrow"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite grow width in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite grow width in shader '%s'\n", shader.name );
			return false;
		}
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite grow height in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite grow height in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssFXAlphaRange"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite fxalpha start in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0 || value > 1.0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite fxalpha start in shader '%s'\n", shader.name );
			return false;
		}
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfacesprite fxalpha end in shader '%s'\n", shader.name );
			return false;
		}
		value = atof(token);
		if (value < 0 || value > 1.0)
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid surfacesprite fxalpha end in shader '%s'\n", shader.name );
			return false;
		}
	...
	if (!Q_stricmp(param, "ssFXWeather"))
	{
		...
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: weather applied to non-effect surfacesprite in shader '%s'\n", shader.name );
			return false;
		}
	...
	// invalid ss command.
	//
	R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid optional surfacesprite param '%s' in shader '%s'\n", param, shader.name );
	return false;
}
...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		if ( !token[0] )
		{
			R_Printf( PRINT_WARNING, "WARNING: no matching '}' found\n" );
			return qfalse;
		}
		...
		else if ( !Q_stricmp( token, "map" ) || (!Q_stricmp(token, "clampmap")))
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for 'map' keyword in shader '%s'\n", shader.name);
				return qfalse;
			}
...
#ifndef FINAL_BUILD
					R_Printf (PRINT_ALL, S_COLOR_RED "Lightmap requested but none avilable for shader '%s'\n", shader.name);
#endif
					stage->bundle[0].image[0] = tr.whiteImage;
				} else {
			...
			else if ( !Q_stricmp( token, "$deluxemap" ) )
			{
				...
				{
					R_Printf( PRINT_WARNING, "WARNING: shader '%s' wants a deluxe map in a map compiled without them\n", shader.name );
					return qfalse;
				}
			...
			else
			{
				...
				{
					R_Printf( PRINT_WARNING, "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n", token, shader.name );
					return qfalse;
				}
		...
		else if (!Q_stricmp(token, "normalMap") || !Q_stricmp(token, "normalHeightMap"))
		{
			...
			if (!token[0])
			{
				R_Printf(PRINT_WARNING, "WARNING: missing parameter for 'normalMap' keyword in shader '%s'\n", shader.name);
				return qfalse;
			}
		...
		else if (!Q_stricmp(token, "specMap") || !Q_stricmp(token, "specularMap"))
		{
			...
			if (!token[0])
			{
				R_Printf(PRINT_WARNING, "WARNING: missing parameter for 'specularMap' keyword in shader '%s'\n", shader.name);
				return qfalse;
			}
		...
		else if (!Q_stricmp(token, "rmoMap") || !Q_stricmp(token, "rmosMap"))
		{
			...
			if (!token[0])
			{
				R_Printf(PRINT_WARNING, "WARNING: missing parameter for 'rmoMap' keyword in shader '%s'\n", shader.name);
				return qfalse;
			}
		...
		else if (!Q_stricmp(token, "moxrMap") || !Q_stricmp(token, "mosrMap"))
		{
			...
			if (!token[0])
			{
				R_Printf(PRINT_WARNING, "WARNING: missing parameter for 'moxrMap' keyword in shader '%s'\n", shader.name);
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "animMap" ) || !Q_stricmp( token, "clampanimMap" ) || !Q_stricmp( token, "oneshotanimMap" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for '%s' keyword in shader '%s'\n", animMapNames[type], shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "videoMap" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for 'videoMap' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "alphaFunc" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for 'alphaFunc' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "depthfunc" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for 'depthfunc' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
			...
			{
				R_Printf( PRINT_WARNING, "WARNING: unknown depthfunc '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
		...
		else if ( !Q_stricmp( token, "blendfunc" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parm for blendFunc in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if (!Q_stricmp(token, "specularreflectance"))
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for specular reflectance in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if (!Q_stricmp(token, "specularexponent"))
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for specular exponent in shader '%s'\n", shader.name );
				continue;
			}
			exponent = atof( token );
		...
		else if ( !Q_stricmp( token, "gloss" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for gloss in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if (!Q_stricmp(token, "roughness"))
		{
			...
			if (token[0] == 0)
			{
				R_Printf(PRINT_WARNING, "WARNING: missing parameter for roughness in shader '%s'\n", shader.name);
				continue;
			}
		...
		else if (!Q_stricmp(token, "parallaxdepth"))
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for parallaxDepth in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if (!Q_stricmp(token, "normalscale"))
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for normalScale in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if (!Q_stricmp(token, "specularscale"))
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for specularScale in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if ( !Q_stricmp( token, "rgbGen" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameters for rgbGen in shader '%s'\n", shader.name );
				continue;
			}
			...
			{
				R_Printf( PRINT_WARNING, "WARNING: unknown rgbGen parameter '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
		...
		else if ( !Q_stricmp( token, "alphaGen" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameters for alphaGen in shader '%s'\n", shader.name );
				continue;
			}
			...
			else if ( !Q_stricmp( token, "portal" ) )
			{
				...
				if ( token[0] == 0 )
				{
					...
					R_Printf( PRINT_WARNING, "WARNING: missing range parameter for alphaGen portal in shader '%s', defaulting to 256\n", shader.name );
				}
				else
				{
			...
			{
				R_Printf( PRINT_WARNING, "WARNING: unknown alphaGen parameter '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
		...
		else if ( !Q_stricmp(token, "texgen") || !Q_stricmp( token, "tcGen" ) ) 
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing texgen parm in shader '%s'\n", shader.name );
				continue;
			}
			...
			{
				R_Printf( PRINT_WARNING, "WARNING: unknown texgen parm in shader '%s'\n", shader.name );
			}
		}
		//
		// tcMod <type> <...>
		...
		{
			R_Printf( PRINT_WARNING, "WARNING: unknown parameter '%s' in shader '%s'\n", token, shader.name );
			return qfalse;
		}
	}
...
static void ParseDeform( const char **text ) {
	...
	if ( token[0] == 0 )
	{
		R_Printf( PRINT_WARNING, "WARNING: missing deform parm in shader '%s'\n", shader.name );
		return;
	}

	if ( shader.numDeforms == MAX_SHADER_DEFORMS ) {
		R_Printf( PRINT_WARNING, "WARNING: MAX_SHADER_DEFORMS in '%s'\n", shader.name );
		return;
	}
	...
	if ( !Q_stricmp( token, "bulge" ) )	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing deformVertexes bulge parm in shader '%s'\n", shader.name );
			return;
		}
	...
	if ( !Q_stricmp( token, "wave" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_WARNING, "WARNING: missing deformVertexes parm in shader '%s'\n", shader.name );
			return;
		}
		...
		else
		{
			...
			R_Printf( PRINT_WARNING, "WARNING: illegal div value of 0 in deformVertexes command for shader '%s'\n", shader.name );
		}
	...
	R_Printf( PRINT_WARNING, "WARNING: unknown deformVertexes subtype '%s' found in shader '%s'\n", token, shader.name );
}
...
static void ParseSkyParms( const char **text ) {
	...
	token = COM_ParseExt( text, qfalse );
	if ( token[0] == 0 ) {
		R_Printf( PRINT_WARNING, "WARNING: 'skyParms' missing parameter in shader '%s'\n", shader.name );
		return;
	}
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
	...
	token = COM_ParseExt( text, qfalse );
	if ( token[0] == 0 ) {
		R_Printf( PRINT_WARNING, "WARNING: 'skyParms' missing cloudheight in shader '%s'\n", shader.name );
		return;
	}
	...
	token = COM_ParseExt( text, qfalse );
	if ( strcmp( token, "-" ) ) {
		R_Printf( PRINT_WARNING, "WARNING: in shader '%s' 'skyParms', innerbox is not supported!", shader.name );
	}
...
void ParseSort( const char **text ) {
	...
	token = COM_ParseExt( text, qfalse );
	if ( token[0] == 0 ) {
		R_Printf( PRINT_WARNING, "WARNING: missing sort parameter in shader '%s'\n", shader.name );
		return;
	}
...
static qboolean ParseShader( const char **text )
{
	...
	if ( token[0] != '{' )
	{
		R_Printf( PRINT_WARNING, "WARNING: expecting '{', found '%s' instead in shader '%s'\n", token, shader.name );
		return qfalse;
	}
	...
	while ( 1 )
	{
		...
		if ( !token[0] )
		{
			R_Printf( PRINT_WARNING, "WARNING: no concluding '}' in shader %s\n", shader.name );
			return qfalse;
		}
		...
		else if ( token[0] == '{' )
		{
			if ( s >= MAX_SHADER_STAGES ) {
				R_Printf( PRINT_WARNING, "WARNING: too many stages in shader %s\n", shader.name );
				return qfalse;
			}
		...
		// sun parms
			...
			if (isGL2Sun)
			{
				...
				{
					R_Printf (PRINT_WARNING, "WARNING: q3gl2_sun's 'shadow scale' value must be between 0 and 1. Clamping to 0.0.\n");
					tr.sunShadowScale = 0.0f;
				}
				else if (tr.sunShadowScale > 1.0f)
				{
					R_Printf (PRINT_WARNING, "WARNING: q3gl2_sun's 'shadow scale' value must be between 0 and 1. Clamping to 1.0.\n");
		...
		else if ( !Q_stricmp( token, "fogParms" ) ) 
		{
			...
			if ( !token[0] ) 
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parm for 'fogParms' keyword in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if ( !Q_stricmp( token, "cull") ) 
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing cull parms in shader '%s'\n", shader.name );
				continue;
			}
			...
			{
				R_Printf( PRINT_WARNING, "WARNING: invalid cull parm '%s' in shader '%s'\n", token, shader.name );
			}
			continue;
		}
		// sort
		...
		{
			R_Printf( PRINT_WARNING, "WARNING: unknown general shader parameter '%s' in '%s'\n", token, shader.name );
			return qfalse;
		}
	}
	...
	qboolean useLightVector, qboolean useLightVertex, qboolean tcgen)
{
	...
	//R_Printf(PRINT_ALL, "shader %s has diffuse %s", shader.name, stage->bundle[0].image[0]->imgName);
	...
	if (r_normalMapping->integer)
	{
		...
		if (stage->bundle[TB_NORMALMAP].image[0])
		{
			...
			//R_Printf(PRINT_ALL, ", normalmap %s", stage->bundle[TB_NORMALMAP].image[0]->imgName);
		}
...
static qboolean CollapseStagesToGLSL(void)
{
	...
	R_Printf (PRINT_DEVELOPER, "Collapsing stages for shader '%s'\n", shader.name);

	// skip shaders with deforms
	if (shader.numDeforms != 0)
	{
		skip = qtrue;
		R_Printf (PRINT_DEVELOPER, "> Shader has vertex deformations. Aborting stage collapsing\n");
	}

	R_Printf (PRINT_DEVELOPER, "> Original shader stage order:\n");
	...
	if (!skip)
	{
			...
			stages[1].active)
		{
				...
				|| blendBits == (GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR))
			{
				...
				R_Printf (PRINT_DEVELOPER, "> Swapped first and second stage.\n");
				R_Printf (PRINT_DEVELOPER, "-> First stage is now: %s\n", stages[0].bundle[0].image[0]->imgName);
				R_Printf (PRINT_DEVELOPER, "-> Second stage is now: %s\n", stages[1].bundle[0].image[0]->imgName);
			}
		}
	}
	...
	R_Printf (PRINT_DEVELOPER, "> New shader stage order:\n");
...
static shader_t *GeneratePermanentShader( void ) {
	...
	if ( tr.numShaders == MAX_SHADERS ) {
		R_Printf( PRINT_WARNING, "WARNING: GeneratePermanentShader - MAX_SHADERS hit\n");
...
static shader_t *FinishShader( void ) {
	...
	for ( stage = 0; stage < MAX_SHADER_STAGES; ) {
		...
		if ( !pStage->bundle[0].image[0] ) {
			R_Printf( PRINT_WARNING, "Shader %s, stage %d has no image. This stage will be ignored\n", shader.name, stage + 1 );
			pStage->active = qfalse;
			stage++;
			continue;
		}
	...
	if ( shader.lightmapIndex[0] >= 0 && !hasLightmapStage ) {
		R_Printf( PRINT_DEVELOPER, "WARNING: shader '%s' has lightmap but no lightmap stage!\n", shader.name );
...
shader_t *R_FindShader( const char *name, const int *lightmapIndexes, const byte *styles, qboolean mipRawImage ) {
	...
	if ( lightmapIndexes[0] >= 0 && lightmapIndexes[0] >= tr.numLightmaps ) {
		...
		R_Printf( PRINT_WARNING, "WARNING: shader '%s' has invalid lightmap index of %d\n", name, lightmapIndexes[0]  );
		lightmapIndexes = lightmapsVertex;
	}
	...
	if ( !image ) {
		R_Printf( PRINT_DEVELOPER, "Couldn't find image file for shader %s\n", name );
		shader.defaultShader = qtrue;
		return FinishShader();
	}
...
qhandle_t RE_RegisterShaderLightMap( const char *name, const int *lightmapIndexes, const byte *styles ) {
	...
	if ( strlen( name ) >= MAX_QPATH ) {
		R_Printf( PRINT_ALL, "Shader name exceeds MAX_QPATH\n" );
		return 0;
	}
...
shader_t *R_GetShaderByHandle( qhandle_t hShader ) {
	if ( hShader < 0 ) {
	  R_Printf( PRINT_WARNING, "R_GetShaderByHandle: out of range hShader '%d'\n", hShader );
		return tr.defaultShader;
	}
...
void	R_ShaderList_f (void) {
	...
	for ( i = 0 ; i < tr.numShaders ; i++ ) {
		...
		R_Printf( PRINT_ALL, "%i ", shader->numUnfoggedPasses );

		if (shader->lightmapIndex[0] >= 0 ) {
			R_Printf (PRINT_ALL, "L ");
		} else {
			R_Printf (PRINT_ALL, "  ");
		}

		if ( shader->explicitlyDefined ) {
			R_Printf( PRINT_ALL, "E " );
		} else {
			R_Printf( PRINT_ALL, "  " );
		}

		if ( shader->optimalStageIteratorFunc == RB_StageIteratorGeneric ) {
			R_Printf( PRINT_ALL, "gen " );
		} else if ( shader->optimalStageIteratorFunc == RB_StageIteratorSky ) {
			R_Printf( PRINT_ALL, "sky " );
		} else {
			R_Printf( PRINT_ALL, "    " );
		}

		if ( shader->defaultShader ) {
			R_Printf (PRINT_ALL,  ": %s (DEFAULTED)\n", shader->name);
		} else {
			R_Printf (PRINT_ALL,  ": %s\n", shader->name);
		}
		count++;
	}
	R_Printf (PRINT_ALL, "%i total shaders\n", count);
	R_Printf (PRINT_ALL, "------------------\n");
}
...
static void ScanAndLoadShaderFiles( void )
{
	...
	{
		R_Printf( PRINT_WARNING, "WARNING: no shader files found\n" );
		return;
	}
	...
	for ( i = 0; i < numShaderFiles; i++ )
	{
		...
		while(1)
		{
			...
			{
				R_Printf(PRINT_WARNING, "WARNING: Ignoring shader file %s. Shader \"%s\" on line %d missing opening brace",
...
void R_InitShaders( qboolean server ) {
	R_Printf( PRINT_ALL, "Initializing Shaders\n" );
