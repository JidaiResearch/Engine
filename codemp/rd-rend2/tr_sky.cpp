...
static void ClipSkyPolygon (int nump, vec3_t vecs, int stage) 
{
	...
	if (nump > MAX_CLIP_VERTS-2)
		R_Error (ERR_DROP, "ClipSkyPolygon: MAX_CLIP_VERTS");
	if (stage == 6)
...
static void DrawSkySide( struct image_s *image, const int mins[2], const int maxs[2] )
{
			...
			t <= maxs[1]+HALF_SKY_SUBDIVISIONS; t++ )
	{
				...
				s <= maxs[0]+HALF_SKY_SUBDIVISIONS; s++ )
		{
			...
			{
				R_Error(ERR_DROP, "SHADER_MAX_VERTEXES hit in DrawSkySideVBO()");
			}
		}
	}
	...
	for ( t = 0; t < tsize; t++ )
	{
		for ( s = 0; s < ssize; s++ )
		{
			...
			{
				R_Error(ERR_DROP, "SHADER_MAX_INDEXES hit in DrawSkySideVBO()");
			}
...
static void FillCloudySkySide( const int mins[2], const int maxs[2], qboolean addIndexes )
{
	...
	for ( t = mins[1]+HALF_SKY_SUBDIVISIONS; t <= maxs[1]+HALF_SKY_SUBDIVISIONS; t++ )
	{
		for ( s = mins[0]+HALF_SKY_SUBDIVISIONS; s <= maxs[0]+HALF_SKY_SUBDIVISIONS; s++ )
		{
			...
			if ( tess.numVertexes >= SHADER_MAX_VERTEXES )
			{
				R_Error( ERR_DROP, "SHADER_MAX_VERTEXES hit in FillCloudySkySide()" );
			}
		}
	}
