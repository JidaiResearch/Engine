...
qboolean R_CheckFBO(const FBO_t * fbo)
{
	...
	switch (code)
	{
		...
		case GL_FRAMEBUFFER_UNSUPPORTED:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Unsupported framebuffer format\n", fbo->name);
			break;

		case GL_FRAMEBUFFER_UNDEFINED:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Default framebuffer was checked, but does not exist\n", fbo->name);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Framebuffer incomplete attachment\n", fbo->name);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Framebuffer incomplete, no attachments attached\n", fbo->name);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Framebuffer incomplete, mismatched multisampling values\n", fbo->name);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Framebuffer incomplete, mismatched layer targets\n",
					  fbo->name);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Framebuffer incomplete, missing draw buffer\n", fbo->name);
			break;

		case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
			R_Printf(PRINT_WARNING, "R_CheckFBO: (%s) Framebuffer incomplete, missing read buffer\n", fbo->name);
			break;
...
void FBO_CreateBuffer(FBO_t *fbo, int format, int index, int multisample)
{
	...
	switch(format)
	{
		...
		default:
			R_Printf(PRINT_WARNING, "FBO_CreateBuffer: invalid format %d\n", format);
			return;
	}
...
void R_AttachFBOTexture1D(int texId, int index)
{
	...
	{
		R_Printf(PRINT_WARNING, "R_AttachFBOTexture1D: invalid attachment index %i\n", index);
		return;
	}
...
void R_AttachFBOTexture2D(int target, int texId, int index)
{
	...
	{
		R_Printf(PRINT_WARNING, "R_AttachFBOTexture2D: invalid target %i\n", target);
		return;
	}

	if (index < 0 || index >= glRefConfig.maxColorAttachments)
	{
		R_Printf(PRINT_WARNING, "R_AttachFBOTexture2D: invalid attachment index %i\n", index);
		return;
	}
...
void R_AttachFBOTexture3D(int texId, int index, int zOffset)
{
	if(index < 0 || index >= glRefConfig.maxColorAttachments)
	{
		R_Printf(PRINT_WARNING, "R_AttachFBOTexture3D: invalid attachment index %i\n", index);
		return;
	}
...
void FBO_AttachTextureImage(image_t *img, int index)
{
	...
	{
		R_Printf(PRINT_WARNING, "FBO: attempted to attach a texture image with no FBO bound!\n");
		return;
	}
...
void FBO_Shutdown(void)
{
	...
	R_Printf(PRINT_ALL, "------- FBO_Shutdown -------\n");
...
void FBO_Blit(FBO_t *src, vec4i_t inSrcBox, vec2_t srcTexScale, FBO_t *dst, vec4i_t dstBox, struct shaderProgram_s *shaderProgram, vec4_t color, int blend)
{
	...
	if (!src)
	{
		R_Printf(PRINT_WARNING, "Tried to blit from a NULL FBO!\n");
		return;
	}
