...
struct PNGFileReader
{
	...
	int Read ( byte **data, int *width, int *height )
	{
		...
		*height = 0;
		...
		{
			R_Printf (PRINT_ERROR, "PNG signature not found in given image.");
			return 0;
		}
		...
		if ( png_ptr == NULL )
		{
			R_Printf (PRINT_ERROR, "Could not allocate enough memory to load the image.");
			return 0;
		}
		...
		{
			R_Printf (PRINT_ERROR, "Width or height is not a power-of-two.\n");
			return 0;
		}
		...
		{
			R_Printf (PRINT_ERROR, "Image is not 24-bit or 32-bit.");
			return 0;
		}
		...
		byte *tempData = (byte *)Z_Malloc (width_ * height_ * 4, TAG_TEMP_PNG, qfalse, 4);
		if ( !tempData )
		{
			R_Printf (PRINT_ERROR, "Could not allocate enough memory to load the image.");
			return 0;
		...
		if ( setjmp (png_jmpbuf (png_ptr)) )
		{
			Hunk_FreeTempMemory (row_pointers);
			Z_Free (tempData);
			return 0;
		}
