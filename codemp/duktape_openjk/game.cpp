...
CCALL int jl_entity_moveto(int id, struct vec3 *to, float durationSeconds) {
...
CCALL void jl_player_set_velocity(int id, float *pos) {
	gentity_t *ent = g_entities + id;
	memcpy(ent->client->ps.velocity, pos, sizeof(float) * 3);
}
...
int moveto(gentity_t *ent, vec3_t to, float durationSeconds) {
	...
	// VectorScale:     C = A * B
	VectorSubtract( ent->r.currentOrigin, to, move );
	...
	ent->s.pos.trTime = level.time;
		...
		VectorSubtract( to, ent->s.pos.trBase, delta );
