...
/* this function should be binded via ETL engine somehow, but then i need to link against Duktape for all the stack functions, which are only in kungcode atm
	potential solutions:
	 - merge everything 
	 - export own wrapper of duktape stack functions like stackPushUndefined() or just the original ones (easier)
	 - link against a common duktape.lib 

*/

//#define DO_EXPORT
//#include <kung/kungcode.h>
...
//int duk_func_get_gentities(duk_context *ctx) {
//	leetmenu_t *leetmenu = leetmenu_fromglobal();
//	//int ret = leetmenu->info_callback((int)"get gentities");
//	if (gvm == NULL) {
//		duk_push_int(ctx, 0);
//		return 1;
//	}
//	//int ret = VM_Call(gvm, GAME_GET_GENTITIES);
//	int ret = (int)g_entities;
//	duk_push_int(ctx, ret);
//	return 1;
//}
//int duk_func_set_velocity_y(duk_context *ctx) {
//	int playerid = duk_to_int(ctx, 0);
//	float x = duk_to_number(ctx, 1);
//	float y = duk_to_number(ctx, 2);
//	float z = duk_to_number(ctx, 3);
//	g_entities[playerid].client->ps.velocity[0] = x;
//	g_entities[playerid].client->ps.velocity[1] = y;
//	g_entities[playerid].client->ps.velocity[2] = z;
//	return 0;
//}
...
int js_init() {
	...
	struct funcis funcs[] = {
		...
		//{"get_gentities",              duk_func_get_gentities             },
		//{"set_velocity_y",              duk_func_set_velocity_y             },
		{NULL, NULL}
	};
	...
	// rudimentary js handling function, which can be easily overwritten by init.js
