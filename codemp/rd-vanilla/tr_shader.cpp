...
void R_RemapShader(const char *shaderName, const char *newShaderName, const char *timeOffset) {
	...
	if (sh == NULL || sh == tr.defaultShader) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: R_RemapShader: shader %s not found\n", shaderName );
		return;
	}
	...
	if (sh2 == NULL || sh2 == tr.defaultShader) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: R_RemapShader: new shader %s not found\n", newShaderName );
		return;
	}
...
qboolean ParseVector( const char **text, int count, float *v ) {
	...
	if ( strcmp( token, "(" ) ) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parenthesis in shader '%s'\n", shader.name );
		return qfalse;
	}
	...
	for ( i = 0 ; i < count ; i++ ) {
		token = COM_ParseExt( text, qfalse );
		if ( !token[0] ) {
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing vector element in shader '%s'\n", shader.name );
			return qfalse;
		}
		v[i] = atof( token );
	}
...
static unsigned NameToAFunc( const char *funcname )
{
	...
	R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid alphaFunc name '%s' in shader '%s'\n", funcname, shader.name );
	return 0;
}
...
static int NameToSrcBlendMode( const char *name )
{
	...
	R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown blend mode '%s' in shader '%s', substituting GL_ONE\n", name, shader.name );
	return GLS_SRCBLEND_ONE;
}
...
static genFunc_t NameToGenFunc( const char *funcname )
{
	...
	R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid genfunc name '%s' in shader '%s'\n", funcname, shader.name );
	return GF_SIN;
}
...
static void ParseWaveForm( const char **text, waveForm_t *wave )
{
	...
	if ( token[0] == 0 )
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing waveform parm in shader '%s'\n", shader.name );
		return;
	}
...
static void ParseTexMod( const char *_text, shaderStage_t *stage )
{
	...
	if ( !Q_stricmp( token, "turb" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing tcMod turb parms in shader '%s'\n", shader.name );
			return;
		}
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing tcMod turb in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "scale" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing scale parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "scroll" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing scale scroll parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "stretch" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing stretch parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "transform" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing transform parms in shader '%s'\n", shader.name );
			return;
		}
	...
	else if ( !Q_stricmp( token, "rotate" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing tcMod rotate parms in shader '%s'\n", shader.name );
			return;
		}
	...
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown tcMod '%s' in shader '%s'\n", token, shader.name );
	}
}
...
static void ParseSurfaceSprites( const char *_text, shaderStage_t *stage )
{
	...
	if (token[0]==0)
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing surfaceSprites params in shader '%s'\n", shader.name );
		return;
	}
	...
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid type in shader '%s'\n", shader.name );
		return;
	}
	...
	if (width <= 0)
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid width in shader '%s'\n", shader.name );
		return;
	}
	...
	if (height <= 0)
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid height in shader '%s'\n", shader.name );
		return;
	}
	...
	if (density <= 0)
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid density in shader '%s'\n", shader.name );
		return;
	}
	...
	if (fadedist < 32)
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid fadedist (%f < 32) in shader '%s'\n", fadedist, shader.name );
		return;
	}
...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		if ( !token[0] )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: no matching '}' found\n" );
			return qfalse;
		}
		...
		else if ( !Q_stricmp( token, "map" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameter for 'map' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
			...
			else if ( !Q_stricmp( token, "$lightmap" ) )
			{
				...
				{
#ifndef FINAL_BUILD
					R_Printf( PRINT_ALL, S_COLOR_RED"Lightmap requested but none available for shader %s\n", shader.name);
#endif
					stage->bundle[0].image = tr.whiteImage;
				}
				else
			...
			else
			{
				...
				{
					R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n", token, shader.name );
					return qfalse;
				}
			}
		}
		...
		else if ( !Q_stricmp( token, "clampmap" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameter for 'clampmap' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "animMap" ) || !Q_stricmp( token, "clampanimMap" ) || !Q_stricmp( token, "oneshotanimMap" ))
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameter for '%s' keyword in shader '%s'\n", (bClamp ? "animMap":"clampanimMap"), shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "videoMap" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameter for 'videoMap' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "alphaFunc" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameter for 'alphaFunc' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "depthfunc" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameter for 'depthfunc' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
			...
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown depthfunc '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
		...
		else if ( !Q_stricmp( token, "blendfunc" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parm for blendFunc in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if ( !Q_stricmp( token, "rgbGen" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameters for rgbGen in shader '%s'\n", shader.name );
				continue;
			}
			...
			else if ( !Q_stricmp( token, "lightingDiffuseEntity" ) )
			{
				...
				{
					R_Printf( PRINT_ALL, S_COLOR_RED "ERROR: rgbGen lightingDiffuseEntity used on a misc_model! in shader '%s'\n", shader.name );
				}
			...
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown rgbGen parameter '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
		...
		else if ( !Q_stricmp( token, "alphaGen" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parameters for alphaGen in shader '%s'\n", shader.name );
				continue;
			}
			...
			else if ( !Q_stricmp( token, "portal" ) )
			{
				...
				if ( token[0] == 0 )
				{
					...
					R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing range parameter for alphaGen portal in shader '%s', defaulting to 256\n", shader.name );
				}
				else
				{
			...
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown alphaGen parameter '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
		...
		else if ( !Q_stricmp(token, "texgen") || !Q_stricmp( token, "tcGen" ) )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing texgen parm in shader '%s'\n", shader.name );
				continue;
			}
			...
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown texgen parm in shader '%s'\n", shader.name );
			}
		}
		//
		// tcMod <type> <...>
		...
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown parameter '%s' in shader '%s'\n", token, shader.name );
			return qfalse;
		}
	}
...
static void ParseDeform( const char **text ) {
	...
	if ( token[0] == 0 )
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing deform parm in shader '%s'\n", shader.name );
		return;
	}

	if ( shader.numDeforms == MAX_SHADER_DEFORMS ) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: MAX_SHADER_DEFORMS in '%s'\n", shader.name );
		return;
	}
	...
	if ( !Q_stricmp( token, "bulge" ) )	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing deformVertexes bulge parm in shader '%s'\n", shader.name );
			return;
		}
	...
	if ( !Q_stricmp( token, "wave" ) )
	{
		...
		if ( token[0] == 0 )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing deformVertexes parm in shader '%s'\n", shader.name );
			return;
		}
		...
		else
		{
			...
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: illegal div value of 0 in deformVertexes command for shader '%s'\n", shader.name );
		}
	...
	R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown deformVertexes subtype '%s' found in shader '%s'\n", token, shader.name );
}
...
static void ParseSkyParms( const char **text ) {
	...
	token = COM_ParseExt( text, qfalse );
	if ( token[0] == 0 ) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: 'skyParms' missing parameter in shader '%s'\n", shader.name );
		return;
	}
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
	...
	token = COM_ParseExt( text, qfalse );
	if ( token[0] == 0 ) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: 'skyParms' missing cloudheight in shader '%s'\n", shader.name );
		return;
	}
	...
	token = COM_ParseExt( text, qfalse );
	if ( strcmp( token, "-" ) ) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: in shader '%s' 'skyParms', innerbox is not supported!", shader.name);
	}
}
...
static void ParseSort( const char **text ) {
	...
	token = COM_ParseExt( text, qfalse );
	if ( token[0] == 0 ) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing sort parameter in shader '%s'\n", shader.name );
		return;
	}
...
void ParseMaterial( const char **text )
{
	...
	if ( token[0] == 0 )
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing material in shader '%s'\n", shader.name );
		return;
	}
...
static qboolean ParseShader( const char **text )
{
	...
	if ( token[0] != '{' )
	{
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: expecting '{', found '%s' instead in shader '%s'\n", token, shader.name );
		return qfalse;
	}
	...
	while ( 1 )
	{
		...
		if ( !token[0] )
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: no concluding '}' in shader %s\n", shader.name );
			return qfalse;
		}
		...
		else if ( token[0] == '{' )
		{
			if ( s >= MAX_SHADER_STAGES ) {
				R_Printf( PRINT_WARNING, "WARNING: too many stages in shader %s (max is %i)\n", shader.name, MAX_SHADER_STAGES );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "fogParms" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing parm for 'fogParms' keyword in shader '%s'\n", shader.name );
				continue;
			}
		...
		else if ( !Q_stricmp( token, "cull") )
		{
			...
			if ( token[0] == 0 )
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: missing cull parms in shader '%s'\n", shader.name );
				continue;
			}
			...
			{
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: invalid cull parm '%s' in shader '%s'\n", token, shader.name );
			}
			continue;
		}
		// sort
		...
		{
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: unknown general shader parameter '%s' in '%s'\n", token, shader.name );
			return qfalse;
		}
	}
...
static shader_t *GeneratePermanentShader( void ) {
	...
	if ( tr.numShaders == MAX_SHADERS ) {
		//R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: GeneratePermanentShader - MAX_SHADERS hit\n");
		R_Printf( PRINT_ALL, "WARNING: GeneratePermanentShader - MAX_SHADERS hit\n");
...
static shader_t *FinishShader( void ) {
    ...
    // check for a missing texture
		if ( !pStage->bundle[0].image ) {
			R_Printf( PRINT_ALL, S_COLOR_YELLOW  "Shader %s has a stage with no image\n", shader.name );
...
/*	if ( lightmapIndex[0] >= 0 && lightmapIndex[0] >= tr.numLightmaps )
	...
	if ( !image ) {
		R_Printf( PRINT_DEVELOPER, S_COLOR_RED "Couldn't find image for shader %s\n", name );
		shader.defaultShader = true;
		return FinishShader();
	}
	//
...
shader_t *R_GetShaderByHandle( qhandle_t hShader ) {
	if ( hShader < 0 ) {
		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "R_GetShaderByHandle: out of range hShader '%d'\n", hShader );
		return tr.defaultShader;
	}
...
void	R_ShaderList_f (void) {
	...
	for ( i = 0 ; i < tr.numShaders ; i++ ) {
		...
		if ( shader->multitextureEnv == GL_ADD ) {
			R_Printf( PRINT_ALL, "MT(a) " );
		} else if ( shader->multitextureEnv == GL_MODULATE ) {
			R_Printf( PRINT_ALL, "MT(m) " );
		} else if ( shader->multitextureEnv == GL_DECAL ) {
			R_Printf( PRINT_ALL, "MT(d) " );
		} else {
			R_Printf( PRINT_ALL, "      " );
		}
...
static void ScanAndLoadShaderFiles( void )
{
	...
	for ( i = 0; i < numShaderFiles; i++ )
	{
		...
		while(1)
		{
			...
			if ( token[0] == '#' )
			{
				R_Printf( PRINT_WARNING, "WARNING: Deprecated shader comment \"%s\" on line %d in file %s.  Ignoring line.\n",
