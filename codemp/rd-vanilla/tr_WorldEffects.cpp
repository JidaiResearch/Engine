...
public:
	...
	void			Cache()
	{
		...
		{
			R_Printf( PRINT_ALL, "WARNING: No Weather Zones Encountered\n");
...
void RB_RenderWorldEffects(void)
{
	...
	else
	{
		...
		if (false)
		{
			R_Printf( PRINT_ALL, "Weather: %d Particles Rendered\n", mParticlesRendered);
		}
	}
}
...
qboolean WE_ParseVector( const char **text, int count, float *v ) {
	...
	if ( strcmp( token, "(" ) ) {
		R_Printf (PRINT_WARNING, "WARNING: missing parenthesis in weather effect\n" );
		return qfalse;
	}
	...
	for ( i = 0 ; i < count ; i++ ) {
		token = COM_ParseExt( text, qfalse );
		if ( !token[0] ) {
			R_Printf (PRINT_WARNING, "WARNING: missing vector element in weather effect\n" );
			return qfalse;
		}
		v[i] = atof( token );
	}
...
void RE_WorldEffectCommand(const char *command)
{
	...
	{
		R_Printf( PRINT_ALL, "Weather Effect: Please enter a valid command.\n" );
		R_Printf( PRINT_ALL, "	die\n" );
		R_Printf( PRINT_ALL, "	clear\n" );
		R_Printf( PRINT_ALL, "	freeze\n" );
		R_Printf( PRINT_ALL, "	zone (mins) (maxs)\n" );
		R_Printf( PRINT_ALL, "	wind\n" );
		R_Printf( PRINT_ALL, "	constantwind (velocity)\n" );
		R_Printf( PRINT_ALL, "	gustingwind\n" );
		//R_Printf( PRINT_ALL, "	windzone (mins) (maxs) (velocity)\n" );
		R_Printf( PRINT_ALL, "	lightrain\n" );
		R_Printf( PRINT_ALL, "	rain\n" );
		R_Printf( PRINT_ALL, "	acidrain\n" );
		R_Printf( PRINT_ALL, "	heavyrain\n" );
		R_Printf( PRINT_ALL, "	snow\n" );
		R_Printf( PRINT_ALL, "	spacedust\n" );
		R_Printf( PRINT_ALL, "	sand\n" );
		R_Printf( PRINT_ALL, "	fog\n" );
		R_Printf( PRINT_ALL, "	heavyrainfog\n" );
		R_Printf( PRINT_ALL, "	light_fog\n" );
		R_Printf( PRINT_ALL, "	outsideshake\n" );
		R_Printf( PRINT_ALL, "	outsidepain\n" );
	}
}
