...
void R_AddMD3Surfaces( trRefEntity_t *ent ) {
	...
	if ( (ent->e.frame >= tr.currentModel->md3[0]->numFrames)
		...
		|| (ent->e.oldframe >= tr.currentModel->md3[0]->numFrames)
		|| (ent->e.oldframe < 0) ) {
			R_Printf( PRINT_DEVELOPER, S_COLOR_RED "R_AddMD3Surfaces: no such frame %d to %d for '%s'\n",
	...
	for ( i = 0 ; i < header->numSurfaces ; i++ ) {
		...
		if ( ent->e.customShader ) {
			...
			if (shader == tr.defaultShader) {
				R_Printf( PRINT_DEVELOPER, S_COLOR_RED "WARNING: no shader for surface %s in skin %s\n", surface->name, skin->name);
			}
			else if (shader->defaultShader) {
				R_Printf( PRINT_DEVELOPER, S_COLOR_RED "WARNING: shader %s in skin %s not found\n", shader->name, skin->name);
			}
