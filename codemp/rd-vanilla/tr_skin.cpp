...
qhandle_t RE_RegisterIndividualSkin( const char *name , qhandle_t hSkin)
{
	...
	if ( !text ) {
#ifndef FINAL_BUILD
		R_Printf( PRINT_ALL, "WARNING: RE_RegisterSkin( '%s' ) failed to load!\n", name );
#endif
		return 0;
	}
	...
	while ( text_p && *text_p ) {
		...
		if ( (unsigned)skin->numSurfaces >= ARRAY_LEN( skin->surfaces ) )
		{
			...
			R_Printf( PRINT_ALL, "WARNING: RE_RegisterSkin( '%s' ) more than %u surfaces!\n", name, (unsigned int )ARRAY_LEN( skin->surfaces ) );
			break;
		}
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	if ( !name || !name[0] ) {
		R_Printf( PRINT_ALL, "Empty name passed to RE_RegisterSkin\n" );
		return 0;
	}

	if ( strlen( name ) >= MAX_QPATH ) {
		R_Printf( PRINT_ALL, "Skin name exceeds MAX_QPATH\n" );
		return 0;
	}
	...
	if ( tr.numSkins == MAX_SKINS ) {
		R_Printf( PRINT_ALL, "WARNING: RE_RegisterSkin( '%s' ) MAX_SKINS hit\n", name );
		return 0;
	}
	tr.numSkins++;
