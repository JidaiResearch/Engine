...
void R_ImageList_f( void ) {
	...
	while ( (image = R_Images_GetNextIteration()) != NULL)
	{
		...
		R_Printf( PRINT_ALL,   "%4i: %4i %4i  %s ",
			i, image->width, image->height, yesno[image->mipmap] );
		switch ( image->internalFormat ) {
		case 1:
			R_Printf( PRINT_ALL, "I    " );
			break;
		case 2:
			R_Printf( PRINT_ALL, "IA   " );
			break;
		case 3:
			R_Printf( PRINT_ALL, "RGB  " );
			break;
		case 4:
			R_Printf( PRINT_ALL, "RGBA " );
			break;
		case GL_RGBA8:
			R_Printf( PRINT_ALL, "RGBA8" );
			break;
		case GL_RGB8:
			R_Printf( PRINT_ALL, "RGB8" );
			break;
		case GL_RGB4_S3TC:
			R_Printf( PRINT_ALL, "S3TC " );
			break;
		case GL_COMPRESSED_RGB_S3TC_DXT1_EXT:
			R_Printf( PRINT_ALL, "DXT1 " );
			break;
		case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
			R_Printf( PRINT_ALL, "DXT5 " );
			break;
		case GL_RGBA4:
			R_Printf( PRINT_ALL, "RGBA4" );
			break;
		case GL_RGB5:
			R_Printf( PRINT_ALL, "RGB5 " );
			break;
		default:
			R_Printf( PRINT_ALL, "???? " );
		}
		...
		case GL_REPEAT:
			R_Printf( PRINT_ALL, "rept " );
			break;
		case GL_CLAMP:
			R_Printf( PRINT_ALL, "clmp " );
			break;
		case GL_CLAMP_TO_EDGE:
			R_Printf( PRINT_ALL, "clpE " );
			break;
		default:
			R_Printf( PRINT_ALL, "%4i ", image->wrapClampMode );
			break;
		}
	...
	R_Printf( PRINT_ALL,  " ---------\n");
	R_Printf( PRINT_ALL,  "      -w-- -h-- -mm- -if- wrap --name-------\n");
	R_Printf( PRINT_ALL,  " %i total texels (not including mipmaps)\n", texels );
	R_Printf( PRINT_ALL,  " %.2fMB total texture mem (not including mipmaps)\n", texBytes/1048576.0f );
...
void RE_RegisterImages_Info_f( void )
{
	...
	while ( (pImage	= R_Images_GetNextIteration()) != NULL)
	{
		R_Printf( PRINT_ALL, "%d: (%4dx%4dy) \"%s\"",iImage, pImage->width, pImage->height, pImage->imgName);
		R_Printf( PRINT_DEVELOPER, S_COLOR_RED ", levused %d",pImage->iLastLevelUsedOn);
		R_Printf( PRINT_ALL, "\n");

		iTexels += pImage->width * pImage->height;
		iImage++;
	}
	R_Printf( PRINT_ALL, "%d Images. %d (%.2fMB) texels total, (not including mipmaps)\n",iNumImages, iTexels, (float)iTexels / 1024.0f / 1024.0f);
	R_Printf( PRINT_DEVELOPER, S_COLOR_RED "RE_RegisterMedia_GetLevel(): %d",RE_RegisterMedia_GetLevel());
}
...
{
	R_Printf( PRINT_DEVELOPER, S_COLOR_RED "RE_RegisterImages_LevelLoadEnd():\n");
	...
	for (AllocatedImages_t::iterator itImage = AllocatedImages.begin(); itImage != AllocatedImages.end(); /* blank */)
	{
		...
		if (pImage->imgName[0] != '*' || strchr(pImage->imgName,'/'))
		{
			...
			if ( pImage->iLastLevelUsedOn != RE_RegisterMedia_GetLevel() )
			{
				// nope, so dump it...
				//
				R_Printf( PRINT_DEVELOPER, S_COLOR_RED "Dumping image \"%s\"\n",pImage->imgName);
...
//	{
//		R_Printf( PRINT_ALL, S_COLOR_YELLOW  "Level uses %d images, old limit was MAX_DRAWIMAGES (%d)\n", iNumImages, MAX_DRAWIMAGES);
//	}

	R_Printf( PRINT_DEVELOPER, S_COLOR_RED "RE_RegisterImages_LevelLoadEnd(): Ok\n");
	...
	if (itAllocatedImage != AllocatedImages.end())
	{
		...
		if ( strcmp( pName, "*white" ) ) {
			if ( pImage->mipmap != !!mipmap ) {
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: reused image %s with mixed mipmap parm\n", pName );
			}
			if ( pImage->allowPicmip != !!allowPicmip ) {
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: reused image %s with mixed allowPicmip parm\n", pName );
			}
			if ( pImage->wrapClampMode != glWrapClampMode ) {
				R_Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: reused image %s with mixed glWrapClampMode parm\n", pName );
			}
		}
...
image_t	*R_FindImageFile( const char *name, qboolean mipmap, qboolean allowPicmip, qboolean allowTC, int glWrapClampMode ) {
	...
	if ( (width&(width-1)) || (height&(height-1)) )
	{
		R_Printf( PRINT_ALL, "Refusing to load non-power-2-dims(%d,%d) pic \"%s\"...\n", width,height,name );
		return NULL;
	}
