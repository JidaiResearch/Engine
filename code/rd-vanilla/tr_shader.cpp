...
static unsigned NameToAFunc( const char *funcname )
{
	...
	R_Printf( PRINT_WARNING, "WARNING: invalid alphaFunc name '%s' in shader '%s'\n", funcname, shader.name );
	return 0;
}
...
static void ParseSurfaceSprites(const char *_text, shaderStage_t *stage )
{
	...
	if (token[0]==0)
	{
		R_Printf( PRINT_WARNING, "WARNING: missing surfaceSprites params in shader '%s'\n", shader.name );
		return;
	}
	...
	{
		R_Printf( PRINT_WARNING, "WARNING: invalid type in shader '%s'\n", shader.name );
		return;
	}
	...
	if (width <= 0)
	{
		R_Printf( PRINT_WARNING, "WARNING: invalid width in shader '%s'\n", shader.name );
		return;
	}
	...
	if (height <= 0)
	{
		R_Printf( PRINT_WARNING, "WARNING: invalid height in shader '%s'\n", shader.name );
		return;
	}
	...
	if (density <= 0)
	{
		R_Printf( PRINT_WARNING, "WARNING: invalid density in shader '%s'\n", shader.name );
		return;
	}
	...
	if (fadedist < 32)
	{
		R_Printf( PRINT_WARNING, "WARNING: invalid fadedist (%f < 32) in shader '%s'\n", fadedist, shader.name );
		return;
	}
...
static void ParseSurfaceSpritesOptional( const char *param, const char *_text, shaderStage_t *stage )
{
	...
	if (!Q_stricmp(param, "ssFademax"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite fademax in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value <= stage->ss->fadeDist)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite fademax (%.2f <= fadeDist(%.2f)) in shader '%s'\n", value, stage->ss->fadeDist, shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssFadescale"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite fadescale in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
	...
	if (!Q_stricmp(param, "ssVariance"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite variance width in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite variance width in shader '%s'\n", shader.name );
			return;
		}
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite variance height in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite variance height in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssHangdown"))
	{
		...
		{
			R_Printf( PRINT_WARNING, "WARNING: Hangdown facing overrides previous facing in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssAnyangle"))
	{
		if (stage->ss->facing != SURFSPRITE_FACING_NORMAL)
		{
			R_Printf( PRINT_WARNING, "WARNING: Anyangle facing overrides previous facing in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssFaceup"))
	{
		if (stage->ss->facing != SURFSPRITE_FACING_NORMAL)
		{
			R_Printf( PRINT_WARNING, "WARNING: Faceup facing overrides previous facing in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssWind"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite wind in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0.0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite wind in shader '%s'\n", shader.name );
			return;
		}
		stage->ss->wind=value;
	...
	if (!Q_stricmp(param, "ssWindidle"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite windidle in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0.0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite windidle in shader '%s'\n", shader.name );
			return;
		}
		stage->ss->windIdle=value;
		return;
	...
	if (!Q_stricmp(param, "ssVertskew"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite vertskew in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0.0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite vertskew in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssFXDuration"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite duration in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value <= 0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite duration in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssFXGrow"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite grow width in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite grow width in shader '%s'\n", shader.name );
			return;
		}
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite grow height in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite grow height in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssFXAlphaRange"))
	{
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite fxalpha start in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0 || value > 1.0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite fxalpha start in shader '%s'\n", shader.name );
			return;
		}
		...
		if (token[0]==0)
		{
			R_Printf( PRINT_WARNING, "WARNING: missing surfacesprite fxalpha end in shader '%s'\n", shader.name );
			return;
		}
		value = atof(token);
		if (value < 0 || value > 1.0)
		{
			R_Printf( PRINT_WARNING, "WARNING: invalid surfacesprite fxalpha end in shader '%s'\n", shader.name );
			return;
		}
	...
	if (!Q_stricmp(param, "ssFXWeather"))
	{
		...
		{
			R_Printf( PRINT_WARNING, "WARNING: weather applied to non-effect surfacesprite in shader '%s'\n", shader.name );
			return;
		}
	...
	// invalid ss command.
	//
	R_Printf( PRINT_WARNING, "WARNING: invalid optional surfacesprite param '%s' in shader '%s'\n", param, shader.name );
	return;
}
...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		else if ( !Q_stricmp( token, "map" ) )
		{
			...
			else if ( !Q_stricmp( token, "$lightmap" ) )
			{
				...
				if ( shader.lightmapIndex[0] < 0 ) {
					stage->bundle[0].image = tr.whiteImage;
#ifndef FINAL_BUILD
					//R_Printf( PRINT_WARNING, "WARNING: $lightmap requested but none available '%s'\n", shader.name );
#endif
				} else {
		...
		else if ( !Q_stricmp( token, "clampmap" ) )
		{
			...
			if ( !token[0] )
			{
				R_Printf( PRINT_WARNING, "WARNING: missing parameter for 'clampmap' keyword in shader '%s'\n", shader.name );
				return qfalse;
			}
		...
		else if ( !Q_stricmp( token, "rgbGen" ) )
		{
			...
			else if ( !Q_stricmp( token, "vertex" ) )
			{
				...
				{
					R_Printf( PRINT_ERROR, "ERROR: rgbGen vertex used on a model! in shader '%s'\n", shader.name );
				}
			...
			else if ( !Q_stricmp( token, "lightingDiffuse" ) )
			{
				if (shader.lightmapIndex[0] != LIGHTMAP_NONE)
				{
					R_Printf( PRINT_ERROR, "ERROR: rgbGen lightingDiffuse used on a misc_model! in shader '%s'\n", shader.name );
				}
			...
			else if ( !Q_stricmp( token, "lightingDiffuseEntity" ) )
			{
				if (shader.lightmapIndex[0] != LIGHTMAP_NONE)
				{
					R_Printf( PRINT_ERROR, "ERROR: rgbGen lightingDiffuseEntity used on a misc_model! in shader '%s'\n", shader.name );
				}
			...
			{
				R_Printf( PRINT_ERROR, "ERROR: unknown rgbGen parameter '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
...
static shader_t *GeneratePermanentShader( void ) {
	...
	if ( tr.numShaders == MAX_SHADERS ) {
		...
		R_Printf( PRINT_WARNING, "WARNING: GeneratePermanentShader - MAX_SHADERS (%d) hit (overflowed by %d)\n", MAX_SHADERS, tr.iNumDeniedShaders);
		return tr.defaultShader;
	}
...
static shader_t *FinishShader( void ) {
	...
	for ( stage = 0; stage < MAX_SHADER_STAGES; ) {
		...
		if ( !pStage->bundle[0].image ) {
			R_Printf( PRINT_WARNING, "Shader %s has a stage with no image\n", shader.name );
			pStage->active = false;
			stage++;
			continue;
		}
...
*/
	...
	if ( !image ) {
		...
		{	//hide these warnings
			R_Printf( PRINT_WARNING, "WARNING: Couldn't find image for shader %s\n", name );
		}
		shader.defaultShader = true;
		return FinishShader();
	}
...
static void SetupShaderEntryPtrs(void)
{
	...
	//R_Printf( PRINT_DEVELOPER, "SetupShaderEntryPtrs(): Stored %d shader ptrs\n",ShaderEntryPtrs_Size() );
}
#endif
